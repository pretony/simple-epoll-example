...
{
    ringbuf_t rb = (ringbuf_t)malloc(sizeof(struct ringbuf_s));
    if (rb) {
...
size_t ringbuf_copy_to_data(void *dst, ringbuf_t src, size_t count)
{
    size_t bytes_used = ringbuf_bytes_used(src);
    if (count > bytes_used)
        return 0;
    ...
    while (nwritten != count) {
        assert(bufend > p);
        size_t n = MIN(bufend - p, count - nwritten);
        memcpy(u8dst + nwritten, p, n);
        p += n;
        nwritten += n;
